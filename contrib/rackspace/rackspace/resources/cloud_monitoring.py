...
class TypedMixIn(object):
    ...
    def __init__(self):
        avs = constraints.CustomConstraint(self._type_constraint)
        self.properties_schema[self.TYPE].constraints.append(avs)
        if self._detail_ref:
...
class NotificationPlan(CloudMonitoringResource):
    ...
    def handle_update(self, json_snippet=None, tmpl_diff=None, prop_diff=None):
        # no updates on plans, but label is marked as updatable
        raise resource.UpdateReplace()

    def handle_delete(self):
        if self.resource_id:
            try:
                self.client().delete_notification_plan(self.resource_id)
...
# FIXME: FIX THIS FOR DOC GENERATION
